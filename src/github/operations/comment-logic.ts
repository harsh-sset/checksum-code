...
export function updateCommentBody(input: CommentUpdateInput): string {
  ...
  // First, remove the "Checksum is working…" or "Checksum is working..." message
  const workingPattern = /Checksum is working[…\.]{1,3}(?:\s*<img[^>]*>)?/i;
  ...
  if (actionFailed) {
    header = "**Checksum encountered an error";
    if (durationStr) {
    ...
    header = `**Checksum finished @${username}'s task`;
    if (durationStr) {
